...
void AddLeaf(Node*& ptr, int value, const string& s, const string& original_s) {
    ...
    // Узел может быть промежуточным, поэтому has_key = false
    if (ptr == NULL) {
    ...
    if (s.empty()) {
        ...
        if (ptr->has_key) {//если у этого узла есть значение(has_key = true) значит путь дублируется, а этого не должно быть
            throw runtime_error("Путь '" + original_s + "' уже занят другим значением");
        }
